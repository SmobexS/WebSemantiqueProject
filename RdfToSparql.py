...
class RdfToSparQL:
    ...
    def generate_insert_query(self, graph):
        insert_query = "INSERT DATA {\n"

        for subj, pred, obj in graph:
            subject = self.format_term(subj)
            predicate = self.format_term(pred)
            obj = self.format_term(obj)
    ...
    def generate_search_query(self,date, time):
    ...
    def get_by_place(self,day, time, type, coordinates, max_distance):
        search_query = (
        ...
        if type == "address":
            latitude, longitude = self.get_lat_long_for_place(coordinates)
        elif type == "geocord":
    ...
    def get_lat_long_for_place(self,coordinates):
