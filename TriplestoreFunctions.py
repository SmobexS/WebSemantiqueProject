...
class TripleStore:
    def __init__(self):
        self.fuseki_endpoint_url = "http://localhost:3030/ProjetWebSementic/"

    def import_data (self) : 
        try:
            triplets = SPARQLWrapper(self.fuseki_endpoint_url + 'query')
    ...
    def insert_data(self, insert_query):
        triplets = SPARQLWrapper(self.fuseki_endpoint_url + 'update')
        triplets.setMethod(POST)
        triplets.setQuery(insert_query)
        triplets.query()
        return(self.import_data(self.fuseki_endpoint_url))
